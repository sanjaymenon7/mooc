$( document ).ready(function() {
	...
	$("#n-interface label").css("text-transform", "initial");
});
...
(function(){
	var demo = new Demo({
		...
		update: function(e){
			floatVal = $("#n-interface input").val();
			$("#n-interface input").val(parseInt(floatVal));
			numPoints = 100;
			...
			var n = this.ui.n.value;
			var alpha = this.ui.alpha.value;
			var zalpha = calc_q(alpha/2);
			$("#zvalue").text(parseFloat(zalpha).toFixed(2));
			//console.log(this.ui.mean2.value);
			//console.log(zalpha);
			//console.log(this.ui.stdev2.value);
			//console.log(Math.sqrt(n));
			var temp =((zalpha*this.ui.stdev2.value)/Math.sqrt(n))
			//console.log(temp);

			var leftinterval = this.ui.mean2.value -temp;
			var rightinterval = this.ui.mean2.value +temp;
			$("#leftinterval").text(leftinterval.toFixed(2));
			$("#rightinterval").text(rightinterval.toFixed(2));

			leftintervalpoint =[[leftinterval,0]]
			rightintervalpoint =[[rightinterval,0]]
			...
			var exponent3 = -0.5 * Math.pow(leftinterval-this.ui.mean2.value,2) / Math.pow(this.ui.stdev2.value,2);
			var exponent4 = -0.5 * Math.pow(rightinterval-this.ui.mean2.value,2) / Math.pow(this.ui.stdev2.value,2);
			...
			i=0;
			for (x = 0; x < y3; x = x+0.01) {
				leftintervalpoint[i] = [leftinterval,x];
				i++;
			}
			i=0;
			for (x = 0; x < y4; x = x+0.01) {
				rightintervalpoint[i] = [rightinterval,x];
				i++;
			}
			data = [
				//{data : d1,lines:{fill: true}},
				{data : d2},
				{data : leftintervalpoint,label: 'Lower Confidence Limit', color: 'green', points:{show: true,radius: 4}},
				{data : rightintervalpoint,label: 'Upper Confidence Limit', color: 'blue', points:{show: true,radius: 4}}
