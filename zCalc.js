    ...
    */
    ...
    function calc_z(zValue,pos)
    {
        if (Math.abs(zValue) > Z_MAX) {
    ...
    function calc_q(qValue) {
        if (qValue < 0 ||
            qValue > 1) {
             $("#zvalue").text("Alpha must be between 0 and 1.");
            
        } else {
            return trimfloat(Math.abs(critz(qValue)), ROUND_FLOAT);
        }
    }
