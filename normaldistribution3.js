	...
	var demo = new Demo({
		...
		update: function(e){
			...
			data = [
				{data : d1},
				{data : d2},
				{data : d3,label: 'Z', color: 'black', lines:{show: true}},
				//{data : d4,label: 'Z2', color: 'black', lines:{show: true} },
				{data : d5,label: 'X', color: 'red', points:{show: true,radius: 1}},
				//{data : d6,label: 'X2', color: 'red', points:{show: true,radius: 1}}
			];
			$("#z1value").text(z1.toFixed(2));
			$("#z2value").text(z2.toFixed(2));
			//$("#z1pvalue").text((1-parseFloat(calc_z(z1))).toFixed(4));
			if(z1 <=0){
				$("#z1pvalue").text((parseFloat(calc_z(z1))).toFixed(4));
			}
			else if(z1>0){
				$("#z1pvalue").text((1-parseFloat(calc_z(z1))).toFixed(4));
			}
