...
(function(){
	var demo = new Demo({
		...
		update: function(e){
			...
			//Cp
			var cp= (x2-x1)/(6*this.ui.stdev2.value)
			//Cpk
			console.log("mean");
			console.log(this.ui.stdev2.value);
			console.log("usl-mean");
			console.log(x2-this.ui.stdev2.value);
			console.log("mean-lsl");
			console.log(this.ui.stdev2.value-x1);
			console.log("min");
			console.log(Math.min((x2-this.ui.stdev2.value),(this.ui.stdev2.value-x1)))
			var cpk =Math.min(Math.abs(x2-this.ui.mean2.value),Math.abs(this.ui.mean2.value-x1))/(3*this.ui.stdev2.value);
			//target
			var trgt = (x1+x2)/2
			//for z1 and z2
			d3 =[[z1,0],[z1,y1]]
			...
			data = [
				...
				{data : target,label: 'Target', color: 'green', points:{show: true,radius: 4}},
				{data : d5,label: 'LSL', color: 'red', points:{show: true,radius: 1}},
				{data : d6,label: 'USL', color: 'blue', points:{show: true,radius: 1}}
			];
			$("#z1value").text(z1);
			$("#z2value").text(z2);
			$("#z1pvalue").text(GetZPercent(z1));
			$("#z2pvalue").text(GetZPercent(z2));
			$("#cpvalue").text(cp.toFixed(2));
			$("#cpkvalue").text(cpk.toFixed(2));
			//var total = GetZPercent(z1) + GetZPercent(z2)
			//$("#zpvaluetotal").text(total);
			// Draw Graph
